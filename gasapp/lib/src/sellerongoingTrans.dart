...
import 'package:gasapp/src/viewcosumerLoc.dart';
import 'package:http/http.dart' as http;
import '../model.dart';
import 'dart:async';
import 'package:flutter/material.dart';

class sellerongoingT extends StatefulWidget {
  sellerongoingTState createState() => sellerongoingTState(value);
  String value;
  String userid;
  bool loading = false;
  String status;
  String type;
  sellerongoingT({this.value});
}

// ignore: camel_case_types
class sellerongoingTState extends State<sellerongoingT> {
  String value;
  sellerongoingTState(this.value);
  var adad;
  Timer timer;
  String userid;
  List data = [];
  Future<List> getData() async {
    var urls =
        Uri.parse("https://gasapp123.000webhostapp.com/getsellerongoing.php");
                          ...
                          Center(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                "${widget.list[i]['start_time']}" == "0"
                                    ? Text("Hurry!! your late",
                                        style: new TextStyle(
                                            fontSize: 25.0, color: Colors.red))
                                    : Text(
                                        "Time Remaining : ${widget.list[i]['start_time']} min's",
                                        style: new TextStyle(
                                            fontSize: 25.0, color: Colors.red))
                              ],
                            ),
                            ...
                            children: <Widget>[
                              ...
                              Container(
                                  ...
                                  child: new RaisedButton(
                                      child: new Text("View location"),
                                      color: Colors.blueAccent,
                                      onPressed: () {
                                        Navigator.push(
                                            context,
                                            MaterialPageRoute(
                                                builder: (context) => new CustomerLoca(
                                                    value: value,
                                                    customerlat:
                                                        "${widget.list[i]['consumer_lat']}",
                                                    customerlong:
                                                        "${widget.list[i]['consumer_longh']}",
                                                    sellerlat:
                                                        "${widget.list[i]['lat']}",
                                                    sellerlong:
                                                        "${widget.list[i]['longhi']}",
                                                    orderID:
                                                        "${widget.list[i]['order_id']}")));
                                      })),
                              // Container(
